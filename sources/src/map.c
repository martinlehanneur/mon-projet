...
void bomb2(int previous,int x,int y)
{
	int actual;
	actual=SDL_GetTicks();
	if (actual-previous>1 && actual-previous <2)
	    		{
	    			window_display_image(sprite_get_banner_bomb3(), x, y);

	    		}
	    	else if (actual-previous>2 && actual-previous <3 )
	    		{
	    			window_display_image(sprite_get_banner_bomb2(), x, y);

	    		}
	    	else if (actual-previous>3 && actual-previous <4)
	    		{
